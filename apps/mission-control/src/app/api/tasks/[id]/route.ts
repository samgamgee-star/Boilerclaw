  ...
  try {
    ...
    if (!existing.workflow_template_id && validatedData.assigned_agent_id) {
      ...
      if (defaultTpl) {
        ...
        populateTaskRolesFromAgents(id, existing.workspace_id, validatedData.assigned_agent_id);
      }
    }

    // If an assignee is newly set and task roles are still empty, auto-populate the default workflow roles.
    if (validatedData.assigned_agent_id && validatedData.assigned_agent_id !== existing.assigned_agent_id) {
      const roleCount = queryOne<{ cnt: number }>('SELECT COUNT(*) as cnt FROM task_roles WHERE task_id = ?', [id]);
      ...
      if (validatedData.assigned_agent_id) {
        ...
        if (agent) {
          ...
          if (existing.status === 'assigned' || nextStatus === 'assigned') {
            ...
            const workflowStage = nextStatus || existing.status;
            const isWorkflowStage = ['testing', 'review', 'verification'].includes(workflowStage);
            const stageIsChanging = nextStatus !== undefined && nextStatus !== existing.status;

            // Only direct-dispatch on assignment-only PATCHes inside an already-active workflow stage.
            // If the request includes any status field, the workflow handoff block below owns dispatch
            // to avoid duplicate task_dispatched events.
            if (isWorkflowStage && !stageIsChanging && !statusWasProvided) {
    ...
    // Never run this on the same PATCH that changes stage; that path is owned by handleStageTransition above.
    if (shouldDispatchWorkflowStage && effectiveAssignedAgentId && !statusWasProvided && (!nextStatus || nextStatus === existing.status)) {
