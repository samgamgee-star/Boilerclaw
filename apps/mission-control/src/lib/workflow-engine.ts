...
import { getDefaultRoleAssignments } from '@/lib/task-role-defaults';
 ...
 */
export function populateTaskRolesFromAgents(taskId: string, workspaceId: string, ownerAgentId?: string | null): void {
  const workflow = getTaskWorkflow(taskId);
  if (!workflow) return;
  ...
  // First try the explicit workspace policy keyed off the task owner.
  const roleMap: Record<string, string> = getDefaultRoleAssignments(taskId, workspaceId, ownerAgentId);

  // Fallback: if anything required by the workflow is still missing, use the old fuzzy matching.
